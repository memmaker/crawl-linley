    ...
    int proximity, int extra, int dur)
{
    ...
    if (see_grid(px, py) && proximity == PROX_NEAR_STAIRS)
    {
        ...
        {
#ifdef JP 
            strcat(info, "Ç™Ç†Ç»ÇΩÇ");
            strcat(info, (stair_gfx == '>' || stair_gfx == '<') ? "äKíiÇÃêÅÇ´î≤ÇØ"
                                                                : "ì¸ÇËå˚");
            strcat(info, "Ç©ÇÁâüÇµÇ‚Ç¡ÇΩÅI");
        ...
        {
#ifdef JP 
            strcat(info, (stair_gfx == '>') ? "Ç™äKíiÇè∏Ç¡ÇƒÇ´ÇΩÅB" :
                         (stair_gfx == '<') ? "Ç™äKíiÇç~ÇËÇƒÇ´ÇΩÅB"
                                            : "Ç™ñÂÇí Ç¡ÇƒÇ´ÇΩÅB");
#else
...
#ifdef JP 
            mpr("Ç†Ç»ÇΩÇÕÇ–Ç∆êÅÇ´ÇÃâåÇñ⁄åÇÇµÇΩÅB");
#else
